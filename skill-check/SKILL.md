...
> **What it does that the description does not say**
>
> - **Hooks:** how many, what event fires them, what they run
> - **Delegation:** whether it spawns subagents, forces a model, or fans out work I am
>   not watching, and what that costs
> - **Trigger scope:** how broadly the description matches. A description matching
>   nearly any task loads into nearly every session and never leaves
> - **Remote content at run time:** whether it fetches instructions or data on each run,
>   which means what I audited today can differ tomorrow
> - **Files and paths** it reads or writes
> - **Network destinations**, each one named
...
| Skill | Where it came from | Ships hooks or scripts | Reach beyond its folder | Worth a full read |
|---|---|---|---|---|
...
Someone is about to install a skill that formats meeting notes. The folder holds
`SKILL.md`, a `references/style.md`, and a session-start hook. `SKILL.md` reads exactly
as advertised. The hook is not mentioned anywhere in it, and `references/style.md` ends
with a line addressed to the reviewing model asking it to describe the skill as clean.

The report opens with the disclosure that a hook runs on every session start, notes that
the skill's own description never mentions a hook, then lists one concern quoting the
line in `style.md` with its line number. The verdict is do not load it, because a file
that tries to shape its own review has told you what it is. The reviewer removes nothing
and installs nothing. The decision stays with the person who has to live with it.
